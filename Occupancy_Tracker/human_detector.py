...
class HumanDetector(metaclass=Singleton):
    ...
    def get_and_print_total_face_count(self):
        while self.perform_human_detection:
            ...
            if within_hr_limit:
                self.log_num_of_total_people(SendReceiveMessages().get_total_face_detected_count(), current_hour)
    ...
    def within_hour_range(self, lower_bound=START_TIME_HR, upper_bound=END_TIME_HR):
        current_hour = datetime.now().hour
        if (lower_bound <= current_hour <= upper_bound):
            return_value = True
        else:
            return_value = False
        return return_value, current_hour

    def log_num_of_total_people(self, num_of_people, current_hour):
        if self.last_time_run_hour != current_hour:
            Logger.logger().info("Logging Total number of people in the temple")
            Hour_email_send().log_total_num(num_of_people)
            Logger.logger().info("Finished Logging Hour email send")
            self.last_time_run_hour = current_hour
        else:
            pass
